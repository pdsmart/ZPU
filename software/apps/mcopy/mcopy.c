...
uint32_t app(uint32_t param1, uint32_t param2)
{
    ...
    } else if (!xatoi(&ptr,  &dstAddr))
    {
        xprintf("Illegal <dst addr> value.\n");
    } else
    {
        xprintf("Copying Memory from %08lx:%08lx -> %08lx\n", startAddr, endAddr, dstAddr);
        for(memAddr=startAddr; memAddr < endAddr; memAddr++, dstAddr++)
        {
            *(uint8_t *)(dstAddr) = *(uint8_t *)(memAddr);
        }
    }
